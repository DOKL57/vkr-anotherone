...
Роли и тестовые пользователи описаны в [docs/employees.md](docs/employees.md).

Коротко:

- `ADMIN` и `WAREHOUSE` могут выполнять складские операции: выдачи, возвраты, ремонты, закупки, приёмку, создание новых позиций через закупку и добавление мероприятий.
- Только `ADMIN` может скрывать прошедшие мероприятия и возвращать скрытые мероприятия в активный список.
- `SOUND_ENGINEER` может смотреть данные и пользоваться AI-помощником, но не может менять склад.

## Telegram bot

В проекте есть второй frontend: обычный Telegram bot, не Mini App.

Bot использует тот же backend, что и сайт: ту же авторизацию, роли, AI-запросы, каталог, мероприятия, выдачи, ремонты и закупки. Если `TELEGRAM_BOT_TOKEN` не задан, старый запуск сайта и API не ломается: в dev bot пропускается, в Docker bot остаётся выключенным.
