...
async function main() {
  ...
  const admin = await one('INSERT INTO "Employee" ("id","fullName","role","phone","email") VALUES (gen_random_uuid()::text,$1,$2::"EmployeeRole",$3,$4) RETURNING id', ["Смирнов Артём Павлович", "ADMIN", "+7 999 111-22-33", "admin@sound.local"]);
  const keeper = await one('INSERT INTO "Employee" ("id","fullName","role","phone","email") VALUES (gen_random_uuid()::text,$1,$2::"EmployeeRole",$3,$4) RETURNING id', ["Ковалёв Игорь Сергеевич", "WAREHOUSE", "+7 999 222-33-44", "store@sound.local"]);
  ...
  const tg = await one('INSERT INTO "TelegramUser" ("id","telegramId","username","firstName","lastName","role","employeeId") VALUES (gen_random_uuid()::text,$1,$2,$3,$4,$5::"EmployeeRole",$6) RETURNING id', ["100100100", "artem_admin", "Артём", "Смирнов", "ADMIN", admin.id]);
  ...
  const session = await one('INSERT INTO "ChatSession" ("id","telegramUserId","title","lastIntent","contextJson") VALUES (gen_random_uuid()::text,$1,$2,$3,$4::jsonb) RETURNING id', [tg.id, "Поиск вокального комплекта", "availability_lookup", JSON.stringify({ requested: ["Shure SM58", "XLR"], project: "Фестиваль Город Звук" })]);
