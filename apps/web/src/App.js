...
export default function App() {
    ...
    const activeProjects = useMemo(() => (data?.projects ?? []).filter((project) => !project.archivedAt), [data]);
    const visibleProjects = useMemo(() => showArchivedProjects ? (data?.projects ?? []) : activeProjects, [activeProjects, data, showArchivedProjects]);
    const pastProjectsToArchive = useMemo(() => activeProjects.filter(isPastProject), [activeProjects]);
